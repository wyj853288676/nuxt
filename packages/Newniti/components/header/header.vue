<template>
    <div class="nwt-header">
        <nwt-sidebar :active="showUIConfig" :config="{ contentWidth: '220px' }">
            ...
            <el-col>
                <el-row type="flex" justify="end">
                    ...
                    <el-link class="header-child" :underline="false">
                        <nwt-clock :time="time"></nwt-clock>
                    </el-link>
                    <!-- page lock -->
                    <el-link @click="lockPage()" class="header-child page-lock-link" :underline="false">
                        <i class="el-icon" :class="
                                pageLocked ? 'el-icon-lock' : 'el-icon-unlock'
                            "></i>
                        {{ pageLocked ? "Unlock" : "Lock" }} Screen
                    </el-link>
                    <!-- settings -->
                    <el-link @click="toggleUIConfig()" class="header-child" :underline="false">
                    ...
                    <!-- log out -->
                    <el-link @click="logout" class="header-child" :underline="false">
                        <a-icon type="logout" class="el-icon" /> log out
                    </el-link>
                    <!-- server connection status -->
                    <el-link class="header-child" :underline="false" id="server-delay-link">
                        <el-popover popper-class="server-delay-popover" placement="bottom-end" trigger="click">
                            <server-delay-chart :chartDatas="serverDelays" :maxY="pingDelay">
                                <template slot="chartHeader">
                                    <h5>
                                        <el-link :underline="false" class="text-sm" style="font-size: inherit">
                                            Average Delay ( last 5 times) :
                                            {{
                                                Number(averageDelay).toFixed(2)
                                            }}
                                            ms
                                        </el-link>
                                    </h5>
                            ...
                            <server-signal slot="reference" class="el-icon" :signalLevel="signalLevel">
                            </server-signal>
                        </el-popover>
                    </el-link>
                </el-row>
...
export default {
    ...
    data() {
        return {
            ...
            delaysLimit: 20,
            serverDelays: [],
            //show ui config menu or not
            showUIConfig: false,
        };
    },
    computed: {
        pageLocked: function () {
            return this.$store.state.page.locked;
        },
    ...
    methods: {
        ...
        toggleUIConfig: function () {
            this.showUIConfig = !this.showUIConfig;
        },
        lockPage: function () {
            if (this.pageLocked) {
                this.$store.commit("page/unlockPage");
            } else {
                this.$store.commit("page/lockPage");
            }
        },
        pingServer: function () {
